...
from cisco.models import ConfigFile
...
def _mirror_ftp_dir(ftp_handle, name, overwrite, guess_by_extension, pattern):
    ...
    for file_data in ftp_handle.mlsd(name):

        item, meta = file_data

        if _is_ftp_dir(ftp_handle, name + "/" + item, guess_by_extension):
            # print("Is directory:" + name+"/"+item)
            _mirror_ftp_dir(ftp_handle, name + "/" + item, overwrite, guess_by_extension, pattern)
        else:
            if configFiles.filter(name=item).exists():
                print('Files exist:'+item)
                continue

            print('Downloading file: '+item)
            # today = datetime.now()
            # date_modified = datetime.strptime(meta.get("modify"), "%Y%m%d%H%M%S")
            # if today.date() == date_modified.date():
            #     selected_files.append(name + "/" + item)
            #     print("Is same date")
...
def download_ftp_tree(ftp_handle, path, destination, pattern=None, overwrite=False, guess_by_extension=True):
    ...
    # print("{:30} {:19} ".format("File Name", "Last Modified", "Size"))
